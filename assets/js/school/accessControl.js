...
function advanceSearch02() {
    ...
    console.log(area)
        ...
        //     }, //提交的参数
        //     jsonp: 'Callback',
        //     url: changeUrl.address + "/School_api?whereFrom=AdvancedSearchCount", //提交到那里 后他的服务
        ...
        //                         'OnePageNum': 20
        //                     }
        //                     myAjax(data02, changeUrl.address + "/School_api?whereFrom=AdvancedSearch", accessControl02) //分页搜索
        //                 }
        //             });
        //         })
...
function accessControl02(msg) {
    ...
    for (var i = 0; i < msg.data.list.length; i++) {
        var imgSrc = msg.data.list[i].schoollogo ? 'http://' + changeUrl.imgAddress + msg.data.list[i].schoollogo : '../assets/img/schoolNoPic.png';
        $('#innerWrap').append(
            ...
            '<p class="submitTime"><span class="glyphicon glyphicon-time"></span> 提交时间：' + zeroToEmpty(msg.data.list[i].loadTime) + '</p>' +
            '</div>' +
            '</div>'
        )
    }
...
function generalSearch03() {
    ...
    $.ajax({
        ...
        url: changeUrl.address + "/School_api?whereFrom=count", //获取搜索的总条数
