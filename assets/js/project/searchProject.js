...
function generalSearch() {
    ...
    $.ajax({
        ...
        url: changeUrl.address + "/Subject_api?whereFrom=count", //获取搜索的总条数
        dataType: "jsonp", //数据类型为jsonp  
        jsonp:   "Callback", //服务端用于接收callback调用的function名的参数  
                ...
                laypage({
                    ...
                    jump: function(obj, first) {
                        ...
                        if (data.countAllRS != 0) {
                            myAjax(data01, changeUrl.address + "/Subject_api?whereFrom=search", createProject)
                        } else {
                            $('#loadgif').hide()
                            $('#floatLayer').hide() //遮罩层
                        }
...
function accessData(fn) {
    ...
    if (jQuery.isEmptyObject(args)) {
        ...
        myAjax(data02, changeUrl.address + '/Subject_api?whereFrom=search', fn)
        ...
        $('#outerWrap').html('')
        myAjax(data01, changeUrl.address + '/Subject_api?whereFrom=search', fn)
    }
}
