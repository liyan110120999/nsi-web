...
$(function() {
    //设置省份数据
    // setProvince();
    setProvince('selProvince', 'selCity');
    setProvince('selProvince1', 'selCity1');
});
...
//设置省份数据
function setProvince(parentID, cityID) {
    //给省份下拉列表赋值
    var option, modelVal;
    var $sel = $("#" + parentID);
    //获取对应省份城市
    ...
    //调用change事件，初始城市信息
    provinceChange(parentID, cityID);
}


//根据选中的省份获取对应的城市
function setCity(provinec, cityID) {
    var $city = $("#" + cityID);
    var proCity,
        option,
        modelVal;
...
//省份选中事件
function provinceChange(parentID, cityID) {
    var $pro = $("#" + parentID);
    setCity($pro.val(), cityID);
}